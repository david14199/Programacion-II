...
class SubMenu:
    ...
    def mostrar_lista(self):
        ...
        while(h):
            ...
            opcion=int(input("Ingrese su opcion a mostrar"))

            if (opcion==1):
                try:
                    persona=open("persona.txt","r")
                    print(persona.read())
                    persona.close()
                except:
                    print("no se encontro el archivo")
            if (opcion==0):
                print("Adios tenga buen dia")
                h=False
            if(opcion==2):
                try:
                    universidad=open("universidad.txt","r")
                    print(universidad.read())
                    universidad.close()
                except:
                    print("no se encontro el archivo")
            if(opcion==3):
