...
def calcular_sjf():
    try:
        ...
        if not (len(tiempo_llegada) == len(tiempo_ejecucion) == len(rafaga_cpu)):
            raise ValueError("Las listas deben tener la misma longitud")
        ...
        for i in range(n):
            if i == 0:
                tiempo_respuesta[i] = rafaga_cpu[i]
            else:
                tiempo_respuesta[i] = rafaga_cpu[i] + tiempo_respuesta[i-1]

            tiempo_espera[i] = tiempo_respuesta[i] - tiempo_llegada[i]

        tiempo_espera_promedio = sum(tiempo_espera) / n
        tiempo_respuesta_promedio = sum(tiempo_respuesta) / n

        messagebox.showinfo("Resultados SJF", 
                            f"Tiempo de Espera: {tiempo_espera}\n"
                            f"Tiempo de Respuesta: {tiempo_respuesta}\n"
                            f"Tiempo de Espera Promedio: {tiempo_espera_promedio:.2f}\n"
                            f"Tiempo de Respuesta Promedio: {tiempo_respuesta_promedio:.2f}")
...
calculo_title = Label(main, text="Cálcular", font=("arial", 16))
...
calcular_btn = Button(main, text="Calcular FIFO", command=calcular_fifo).place(x=380, y=550)
calcular_btn_sjf = Button(main, text="Calcular SJF", command=calcular_sjf).place(x=380, y=575)
